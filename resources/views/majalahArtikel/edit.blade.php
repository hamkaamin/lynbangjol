    ...
    <div class="container-fluid">
      <div class="row-fluid" style="margin-bottom: 0px">
        <div class="span4">
            ...
            <h3 style="margin-left: 50px">Edit <strong>ARTIKEL - MAJALAH</strong></h3>
        </div>
        <div class="span8">
    ...
    <div class="container-fluid">
      <div class="row-fluid">
        <div class="span12">
            ...
            <form action="{{ route('majalahArtikel.update', $majalahArtikels->id) }}" method="post" enctype="multipart/form-data" class="comment-form" name="comment-form" onsubmit="return confirm('Konfirmasi Perubahan Data');"> 
            {{ method_field("PUT") }} 
            {!! csrf_field() !!}
            
            <div class="span5">
                <label for="majalah_id" class="control-label">Nama Majalah</label>
                <select id="majalah_id" class="form-control" name="majalah_id" class="selectpicker form-control" data-style="btn-primary" data-live-search="true" data-dropup-auto="false" required autofocus>
                    @foreach ($majalahs as $key => $majalah)
                        @if( $majalah->id  == $majalahArtikels->majalah_id )
            ...
            <div class="clear"></div>
                ...
                <input id="judul" type="text" class="form-control" name="judul" value="{{ $majalahArtikels->judul }}" required />
            <div class="clear"></div>
            
                <label for="halaman" class="control-label">Halaman pada Majalah</label>
                <input id="halaman" type="text" class="form-control" name="halaman" value="{{ $majalahArtikels->halaman}}" />
            </div>
            ...
            <div class="span5">       
                ...
                <select id="author_id" name="author_id[]" class="selectpicker form-control" data-style="btn-primary" data-live-search="true" data-dropup-auto="false" data-selected-text-format="count > 2" multiple title="Pilih Anggota Tim Penelitian">
                    @php($arrTemp = array())
                    @foreach($authorSelected as $majalahAuthor)
                    @php($arrTemp[] = $majalahAuthor->author_id)
                    @endforeach
            ...
            <div class="span6" style="text-align: center">
                ...
                <a class="btn btn-color" style="line-height: 30px; font-size: 14px" href="{{ url('majalahArtikel') }}">
                    Kembali</a>
            </div>
            <div class="clear"></div>
